...
const shouldTryCompatibilityFallback = (status) => [400, 404, 405, 415, 422].includes(status);
...
const createSseResponseFromText = (text, status = 200) => {
  ...
  const body = new ReadableStream({
    start(controller) {
      controller.enqueue(
        encoder.encode(
          `data: ${JSON.stringify({ choices: [{ index: 0, delta: { role: 'assistant', content: safeText } }] })}\n\n`
        )
      );
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && pathname === '/chat') {
      ...
      let fallbackResponse = upstream;

      if (shouldTryCompatibilityFallback(upstream.status)) {
        const chatJsonResponse = await fetch(`${getBaseUrl(env)}/chat/completions`, {
          method: 'POST',
          headers: {
            ...getApiHeaders(env),
            'Content-Type': 'application/json',
        ...
        if (chatJsonResponse.ok) {
          const chatJson = await chatJsonResponse.json();
          return withCors(createSseResponseFromText(extractResponseText(chatJson), chatJsonResponse.status), request, env);
        }

        fallbackResponse = chatJsonResponse;

        const completionResponse = await fetch(`${getBaseUrl(env)}/completions`, {
          method: 'POST',
          headers: {
            ...getApiHeaders(env),
            'Content-Type': 'application/json',
        ...
        if (completionResponse.ok) {
          ...
          return withCors(createSseResponseFromText(extractResponseText(completionJson), completionResponse.status), request, env);
        }

        fallbackResponse = completionResponse;
      }

      return withCors(
        await normalizedUpstreamError(fallbackResponse, 'Reply unavailable right now. Please try another model.'),
        request,
        env
      );
    }
